...
version: "0.1.0"
description: US stock and crypto trading via Alpaca API. Paper trading (simulated) and real trading supported. Real-time quotes, orders, positions, RSI strategy, chat-callable commands, and notify-only setup monitoring for paper trade ideas.
metadata:
  {
    "openclaw":
      {
...
5. Default monitoring mode is notify-only for paper accounts unless the user explicitly requests a trade
...
node {baseDir}/scripts/monitor.js                   # Scan watchlist for paper trade ideas
```
...
- Notify-only watchlist monitoring for paper trade ideas
- Suggested sizing by confidence level
...
4. Optional monitoring config lives at `config.monitor.json`

## Chat Call Patterns

Use these natural chat triggers:
...
- `alpaca quote AAPL`
- `alpaca buy AAPL 1`
- `alpaca buy-amount AAPL 500`
- `alpaca sell AAPL 1`
- `alpaca sell-all AAPL`
- `alpaca check setups`
- `alpaca monitor on`
- `alpaca monitor off`
...
The bundled `config.monitor.json` is set up for:
- paper account only
- notify only
- every 30 minutes
- stocks: AAPL, NVDA, TSLA, SPY, QQQ, AMD, META, MSFT
- crypto: BTCUSD, ETHUSD, SOLUSD, DOGEUSD
- confidence-based sizing suggestions

Suggested sizing defaults:
- Stocks: high $1000, medium $500, low $250
- Crypto: high $500, medium $250, low $100
